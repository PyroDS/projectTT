...
- **Transcript review** (`ui/reviewer.py`): Browse past sessions, view transcripts with speaker-colored text, switch between original and batch-transcribed versions, trigger re-transcription with progress tracking, launch a built-in reviewer walkthrough (auto-shown on open by default as a reviewer-scoped dynamic tour with one floating card, in-window high-contrast target highlighting, moving step placement, persisted opt-out, and toolbar Help button), and provide pyannote setup shortcuts (`Accept Terms`, `HF Token`, `Create Token`) that open Hugging Face in the browser for model terms/token management only.
...
- **Speaker diarization** (`diarizer.py`): Post-processing speaker identification using local neural speaker embeddings (speechbrain/pyannote/resemblyzer) + `scikit-learn` AgglomerativeClustering. Supports three audio source modes: `system` (loopback audio, preserves "You"), `mixed` (single `mic.wav` or imported file, relabels all segments), and `auto` (picks system when enough non-"You" segments exist, otherwise mixed). Splits speakers into "Speaker 1", "Speaker 2", etc. When transcript JSON sidecars include Whisper word timings, diarization aligns each word to the speaker timeline and splits segments at turn changes; older transcripts without word metadata fall back to whole-segment majority voting. User can assign real names through inline UI panel. Produces diarized versioned transcripts, JSON sidecars, and `speaker_map.json`. Supports multi-loopback sessions via `device_manifest.json` WAV discovery and aggregates embeddings from all loopback WAV files in system mode.
