...
- **Transcript review** (`ui/reviewer.py`): Browse past sessions, view transcripts with speaker-colored text, switch between original and batch-transcribed versions, trigger re-transcription with progress tracking, and launch a built-in reviewer walkthrough (auto-shown on open by default as a reviewer-scoped dynamic tour with one floating card, in-window high-contrast target highlighting, moving step placement, persisted opt-out, and toolbar Help button).
