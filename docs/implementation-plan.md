...
- **Speaker diarization** (`diarizer.py` + `diarization/`): Post-processing speaker identification with two families:
  ...
  - **Community-1 backend** (`pyannote_community`): modular local path using `pyannote/speaker-diarization-community-1`, exclusive speaker turns, and word-level assignment. Downloads/caches model locally after HF terms/token setup; runs inference locally. Legacy `pyannote` embedding backend remains on `pyannote.audio==3.4.0`; Community-1 uses `pyannote.audio` 4.x via separate on-demand install.
