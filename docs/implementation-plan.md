...
- **Speaker diarization** (`diarizer.py` + `diarization/`): Post-processing speaker identification with two families:
  - **Embedding backends** (`speechbrain`, `pyannote`, `resemblyzer`): local embeddings + clustering + word/timeline alignment
  - **Community-1 backend** (`pyannote_community`): modular local path using `pyannote/speaker-diarization-community-1`, exclusive speaker turns, and word-level assignment. Downloads/caches the SHA-pinned model locally after HF terms/token setup with `Pipeline.from_pretrained(repo, revision=sha, token=...)`; runs inference locally on preloaded mono 16 kHz waveform input (`{"waveform": tensor, "sample_rate": 16000}`) instead of asking pyannote to decode WAV paths. Legacy `pyannote` embedding backend remains on `pyannote.audio==3.4.0`; Community-1 uses `pyannote.audio` 4.x via separate on-demand install.
