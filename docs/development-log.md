...
- **pyannote.audio install pinning**: `src/tachyon/ui/reviewer.py:_install_pyannote()` now installs `pyannote.audio==3.4.0` (constant `_PYANNOTE_PINNED_SPEC`) instead of the unbounded `pyannote.audio`, eliminating the "compromised PyPI account = arbitrary code execution" path on the on-demand install. The install-prompt also names the pinned version so the user sees what they're agreeing to. 3.4.0 is the last release in the pyannote.audio 3.x line — 4.x introduces breaking changes (Inference API, switches from soundfile/sox to torchcodec for audio I/O) that are untested against the existing diarizer.
...
- **pyannote pinned to 3.4.0 not 4.x.** The current diarizer uses `Model.from_pretrained("pyannote/embedding", token=...)` and `Inference(model, window="whole")`, both stable across 3.x. 4.x is the latest on PyPI but introduces API changes (Inference now requires already-instantiated models — fine for us — and switches from soundfile/sox to torchcodec, which is a fresh native dep with unknown Windows behavior). 3.4.0 is the last release in the 3.x line and changelog notes "only dependency pinning, no user-facing changes" vs. 3.3.x — strictly newer than 3.3.2 with no API risk.
